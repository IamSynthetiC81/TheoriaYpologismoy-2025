...
const Bookstore ctor_Bookstore = { .putOrder=putOrder, .addBook=addBook, .printBookStoreBooks=printBookStoreBooks, .calculateTotalOrdersIncome=calculateTotalOrdersIncome, .listOfBooks[100]=ctor_Book, .listOfOrders[100]=ctor_Order};
#undef SELF;
int orderId;
...
a.setAddress(s, n, c);
return a; };
Person createPerson (, char* fn, char* ln, char* email, int phone, Address addr= ctor_Address) { 
Person p= ctor_Person;
p.setPerson(fn, ln, email, phone, addr);
return p; };
Book createBook (, char* t, Person a= ctor_Person, int numOfCopies, double price) { 
Book b= ctor_Book;
b.setBook(t, a, numOfCopies, price);
return b; };
Order createOrder (, int orNum, Book b= ctor_Book, int q, Address sh= ctor_Address, int del) { 
